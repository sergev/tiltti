...
std::optional<TestCase> MooParser::next()
{
    while (true) {
        ...
        if (std::strcmp(chunk_type, "MOO ") == 0) {
            if (!header_printed_) {
                if (chunk_len < 12) {
                    throw std::runtime_error("Invalid MOO chunk: too short");
                }
                char cpu_name[5] = { 0 };
                ...
                if (std::strcmp(cpu_name, "386E") != 0) {
                    throw std::runtime_error(std::string("Unsupported CPU type: ") + cpu_name +
                                             " (386E required)");
                }
        ...
        //std::cout << "Unknown top-level chunk '" << chunk_type << "' (" << chunk_len << " bytes), skipping\n";
    }
}
